...
"""Best Judgement bars (agent vs bypass) by difficulty, per model.
...
best_judgement rate (rate of True over valid rows).
...
- Figures are saved to results/figures/ by default, one PNG per difficulty per
  model, e.g.: {name}_{model}_bj_by_difficulty_all.png

CLI examples:
    # For all models present in the CSV (one PNG per model)
    python -m src.analysis.figures.bj_by_difficulty_method \
...
def _slugify(s: object) -> str:
    try:
        ...
        return s2 or "bj_by_difficulty"
    except Exception:
        return "bj_by_difficulty"
...
def _coerce_bj(series: pd.Series) -> pd.Series:
    """Coerce best_judgement-like column to boolean then float (0/1)."""
    s = series
    if pd.api.types.is_bool_dtype(s) or isinstance(s.dtype, pd.BooleanDtype):  # type: ignore[attr-defined]
...
def _render_grid_for_model(
    ...
    *,
    ...
    """Render a 1×4 grid of BJ bars (agent vs bypass) for All/Easy/Medium/Hard."""
    has_difficulty = "difficulty" in df_model.columns

    def _panel_stats(sub: pd.DataFrame) -> tuple[pd.DataFrame, int]:
        if sub is None or sub.empty:
            return pd.DataFrame({"method_cat": [], "bj_rate": []}), 0
        work = pd.DataFrame({
            "method_cat": sub["eval_method"].map(_method_to_category),
            "bj": _coerce_bj(sub["best_judgement"]) if "best_judgement" in sub.columns else np.nan,
        })
        # Require a mapped category and a valid BJ (non-NA after coercion)
        work = work.dropna(subset=["method_cat", "bj"])  # type: ignore[arg-type]
        if work.empty:
            return pd.DataFrame({"method_cat": [], "bj_rate": []}), 0
        stats = (
            work.groupby("method_cat")["bj"].agg(["mean", "count"]).reset_index().rename(columns={"mean": "bj_rate"})
        )
    ...
    save_path = output_dir / f"{slug}_{mslug}_bj_by_difficulty.png"
    fig.suptitle(f"{name} — {model_name} — Best Judgement rate by difficulty (agent vs bypass)", y=0.995)
...
def render_bj_by_difficulty_per_model(
    input_csv: Path | str,
    name: str,
    *,
    ...
    """Render four BJ figures (All/Easy/Medium/Hard) for a given model or all models.
    ...
    if "best_judgement" not in df.columns:
        raise ValueError("Column 'best_judgement' not found in input CSV; required to compute BJ rate.")
...
def main(flags: Flags) -> None:
    if flags.split_by_model and flags.model_name is None:
        ...
        render_bj_by_difficulty_per_model(
            flags.input_csv,
            flags.name,
