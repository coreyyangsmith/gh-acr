...
_PROMPT_PATH = Path(__file__).resolve().parents[2] / "prompts" / "bypass2" / "conflict_judge_prompt.txt"
...
def conflict_analyzer_node(state: Dict[str, Any]) -> Dict[str, Any]:  # noqa: D401
    ...
    original_code = "\n\n".join(f"{p}: {ancestor_contents.get(p, '')}" for p in summaries.keys())
    ...
    if llm is None:
        ...
        prompt_text = _PROMPT_STR.format(a_summary=a_sum, b_summary=b_sum, a_diff=a_diff, b_diff=b_diff, original_code=original_code)
