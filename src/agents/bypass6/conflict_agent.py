...
_PLAN_PROMPT_PATH = (
    Path(__file__).resolve().parents[2] / "prompts" / "bypass6" / "plan_prompt.txt"
)
...
def conflict_agent_node(state: Dict[str, Any]) -> Dict[str, Any]:  # noqa: D401
    ...
    if llm is None:
        ...
        a_sum = "\n\n".join(
            f"{p}: {s.get('summary_a', '')}" for p, s in summaries.items()
        )
        b_sum = "\n\n".join(
            f"{p}: {s.get('summary_b', '')}" for p, s in summaries.items()
        )
        a_diff = "\n\n".join(
            f"{p}: {state.get('diffs_a', {}).get(p, '')}" for p in summaries.keys()
        )
        b_diff = "\n\n".join(
            f"{p}: {state.get('diffs_b', {}).get(p, '')}" for p in summaries.keys()
        )
        ...
        try:
            ...
            logger.error(
                "Failed to parse LLM output as JSON, falling back to heuristic."
            )
