...
_MERGE_PROMPT_PATH = (
    Path(__file__).resolve().parents[2] / "prompts" / "bypass6" / "resolver_prompt.txt"
)
    ...
    for path in scenario_files:
        ...
        if choice in ("A", "B") or llm is None:
            if llm is None and choice not in ("A", "B"):
                logger.warning(
                    f"No LLM backend available to merge {path}, falling back to parent A."
                )
                choice = "A"
        ...
        if feedback_text:
            logger.info(
                f"Applying review feedback for {path} (length={len(feedback_text)} chars)"
            )
