...
_PROMPT_PATH = Path(__file__).resolve().parents[2] / "prompts" / "bypass_only" / "conflict_judge_prompt.txt"
...
def conflict_analyzer_node(state: Dict[str, Any]) -> Dict[str, Any]:  # noqa: D401
    ...
    if llm is None:
        ...
        # Up to 3 total attempts (first + 2 retries) to obtain valid "A" or "B"
        attempts = 0
        decision = "INVALID"
        raw_output = ""
        while attempts < 3 and decision == "INVALID":
            ...
            # Attribute tokens for each attempt
        ...
        # If still invalid after retries, default to ALL_A and mark default flag
        if decision == "INVALID":
            decision = "ALL_A"
    ...
    state["bypass_method"] = "A" if decision == "ALL_A" else "B"
