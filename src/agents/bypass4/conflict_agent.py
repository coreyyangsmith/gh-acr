...
_PLAN_PROMPT_PATH = Path(__file__).resolve().parents[2] / "prompts" / "bypass4" / "plan_prompt.txt"
...
def conflict_agent_node(state: Dict[str, Any]) -> Dict[str, Any]:  # noqa: D401
    ...
    if llm is None:
        ...
        try:
            ...
            logger.error("Failed to parse LLM output as JSON, defaulting to merge plan for all files.")
            plan = {p: "merge" for p in summaries}
