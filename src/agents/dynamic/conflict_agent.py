"""Dynamic conflict agent – generates a merge plan AND per-file prompts.

Outputs in state:
- conflict_plan: Dict[path, choice]
- dynamic_prompts: Dict[path, prompt_text]
"""
...
_PLAN_PROMPT_PATH = Path(__file__).resolve().parents[2] / "prompts" / "dynamic" / "plan_prompt.txt"
...
def conflict_agent_node(state: Dict[str, Any]) -> Dict[str, Any]:  # noqa: D401
    logger.info("Dynamic conflict agent started.")
    ...
    if llm is None:
        ...
        for path, s in summaries.items():
            ...
            prompts_by_path[path] = ""  # no dynamic prompt available
    else:
        logger.info("Generating merge plan and dynamic prompts using LLM.")
        ...
        try:
            ...
            plan = data.get("plan", {})
            prompts_by_path = data.get("prompts", {}) or {}
            if not isinstance(plan, dict):
                raise ValueError("plan is not a dict")
            if not isinstance(prompts_by_path, dict):
                prompts_by_path = {}
        except Exception:
            logger.error("Failed to parse LLM output as JSON; falling back to heuristic plan and empty prompts.")
            plan = {p: "merge" for p in summaries}
            prompts_by_path = {}
        # Token accounting across files
    ...
    logger.info(f"Generated merge plan (dynamic): {plan}")
    state["conflict_plan"] = plan or {}
    state["dynamic_prompts"] = prompts_by_path or {}
    state["status"] = "planned_dynamic"
    logger.info("Dynamic conflict agent finished.")
    return state
