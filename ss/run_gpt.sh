#!/bin/bash
#SBATCH --job-name=gpt-gh-acr
#SBATCH --output=logs/gpt-gh-acr-%j.out
#SBATCH --error=logs/gpt-gh-acr-%j.err
...
srun --export=ALL python -m src.cli.run_all \
  ...
  --mode clone \
  --model-name local:openai/gpt-oss-20b \
  --results-filename 2025_09_10_results_gpt_oss_20b_easy.csv
