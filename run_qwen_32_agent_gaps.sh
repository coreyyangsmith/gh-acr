...
#
# Submit:
#   sbatch run_qwen_32_agent_gaps.sh
#
#SBATCH --job-name=qwen32-agent-gaps
#SBATCH --output=logs/qwen32-agent-gaps-%j.out
#SBATCH --error=logs/qwen32-agent-gaps-%j.err
...
if [[ ! -f "$GAP_SUBSET" ]]; then
  echo "ERROR: missing $GAP_SUBSET" >&2
  exit 1
fi

export DATASET_CSV="${SLURM_SUBMIT_DIR:-$(pwd)}/$GAP_SUBSET"
# Local 32B: keep scenario concurrency at 1 (model already fills 2×H100).
export INFERENCE_CONCURRENCY=1

N_SCENARIOS=$(($(wc -l < "$GAP_SUBSET") - 1))
RESULTS_FILE="$(date +%Y_%m_%d)_local_qwen32_agent_gaps_base_agent.csv"
...
echo "Results CSV: data/$RESULTS_FILE"
echo "=================================================================="
